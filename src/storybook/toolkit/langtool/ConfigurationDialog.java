...
public class ConfigurationDialog implements ActionListener {
	...
	public void show(List<Rule> rules) {
		...
		for (final Rule rule : rules) {
			...
			if (prevID == null || !rule.getId().equals(prevID)) {
				...
				if ((showHeadline || prevCategory == null)
					...
					final JCheckBox categoryCheckBox = new JCheckBox(rule
							.getCategory().getName());
					...
					categoryCheckBox
							.addActionListener(categoryCheckBoxListener);
		...
		scroller.getHorizontalScrollBar().setBlockIncrement(10);
		contentPane.add(scroller, cons);
...
class CategoryComparator implements Comparator<Rule> {
	...
	public int compare(final Rule r1, final Rule r2) {
		final boolean hasCat = r1.getCategory() != null
				&& r2.getCategory() != null;
		if (hasCat) {
